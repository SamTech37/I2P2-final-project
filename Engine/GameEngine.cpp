...
void GameEngine::initAllegro5() {
    ...
    if (icon) {
        ...
        LOG(LOG_INFO) << "Loaded window icon from: " << icon;
    }
    ...
    LOG(LOG_INFO) << "There are total " << m_buttons << " supported mouse buttons";
    ...
    while (!done) {
        ...
        switch (event.type) {
            case ALLEGRO_EVENT_DISPLAY_CLOSE:
                ...
                LOG(LOG_VERBOSE) << "Window close button clicked";
                done = true;
                break;
            ...
            case ALLEGRO_EVENT_KEY_DOWN:
                ...
                LOG(LOG_VERBOSE) << "Key with keycode " << event.keyboard.keycode << " down";
                activeScene->OnKeyDown(event.keyboard.keycode);
                break;
            case ALLEGRO_EVENT_KEY_UP:
                // Event for keyboard key up.
                LOG(LOG_VERBOSE) << "Key with keycode " << event.keyboard.keycode << " up";
                activeScene->OnKeyUp(event.keyboard.keycode);
                break;
            case ALLEGRO_EVENT_MOUSE_BUTTON_DOWN:
                // Event for mouse key down.
                LOG(LOG_VERBOSE) << "Mouse button " << event.mouse.button << " down at (" << event.mouse.x << ", " << event.mouse.y << ")";
            ...
            case ALLEGRO_EVENT_MOUSE_AXES:
                if (event.mouse.dx != 0 || event.mouse.dy != 0) {
                    ...
                    LOG(LOG_VERBOSE) << "Mouse move to (" << event.mouse.x << ", " << event.mouse.y << ")";
                    activeScene->OnMouseMove(event.mouse.x, event.mouse.y);
                } else if (event.mouse.dz != 0) {
                    // Event for mouse scroll.
                    LOG(LOG_VERBOSE) << "Mouse scroll at (" << event.mouse.x << ", " << event.mouse.y << ") with delta " << event.mouse.dz;
                    activeScene->OnMouseScroll(event.mouse.x, event.mouse.y, event.mouse.dz);
                }
                break;
            case ALLEGRO_EVENT_MOUSE_LEAVE_DISPLAY:
                LOG(LOG_VERBOSE) << "Mouse leave display.";
            ...
            case ALLEGRO_EVENT_MOUSE_ENTER_DISPLAY:
                LOG(LOG_VERBOSE) << "Mouse enter display.";
                break;
            default:
        ...
        if (redraws > 0 && al_is_event_queue_empty(event_queue)) {
            if (redraws > 1)
                LOG(LOG_VERBOSE) << redraws - 1 << " frame(s) dropped";
...
void GameEngine::changeScene(const std::string& name) {
    ...
    LOG(LOG_INFO) << "Changed to " << name << " scene";
}
void GameEngine::Start(const std::string& firstSceneName, int fps, int screenW, int screenH,
    ...
    LOG(LOG_INFO) << "Game Initializing...";
    ...
    initAllegro5();
    LOG(LOG_INFO) << "Allegro5 initialized";
    LOG(LOG_INFO) << "Game begin";

    activeScene->Initialize();
    LOG(LOG_INFO) << "Game initialized";
    // Draw the first frame.
    draw();
    LOG(LOG_INFO) << "Game start event loop";
    // This call blocks until the game is finished.
    startEventLoop();
    LOG(LOG_INFO) << "Game Terminating...";
    activeScene->Terminate();
    LOG(LOG_INFO) << "Game terminated";
    LOG(LOG_INFO) << "Game end";
    destroy();
}
